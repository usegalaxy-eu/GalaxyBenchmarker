...
class ColdWarmBenchmark(BaseBenchmark):
    ...
    def __init__(
        ...
        name,
        benchmarker,
        destinations: List[Union[PulsarMQDestination, GalaxyDestination]],
        workflows: List[GalaxyWorkflow],
        galaxy,
        repetitions=1,
    ):
    ...
    def run(self, benchmarker):
        ...
        for run_type in ["cold", "warm"]:
            try:
                self.benchmark_results[run_type] = self.run_galaxy_benchmark(
                    self,
                    benchmarker.glx,
                    self.destinations,
                    self.workflows,
...
class DestinationComparisonBenchmark(BaseBenchmark):
    ...
    def run(self, benchmarker):
        ...
        """
        try:
            self.benchmark_results["warm"] = self.run_galaxy_benchmark(
                self,
                benchmarker.glx,
                self.destinations,
                self.workflows,
...
class BurstBenchmark(BaseBenchmark):
    ...
    def __init__(
        ...
        if len(self.destinations) != 1:
            raise ValueError(
                "BurstBenchmark can only be used with exactly one Destination."
            )

        if len(self.workflows) != 1:
            raise ValueError(
                "BurstBenchmark can only be used with exactly one Workflow."
            )
            ...
            for destination in self.destinations:
                for workflow in self.workflows:
                    if type(workflow) is not CondorWorkflow:
                        raise ValueError(
                            "CondorDestination can only work with CondorWorkflow!"
                        )
    ...
    def run(self, benchmarker):
        ...
        for process in threads:
            ...
            finished_jobs += 1
            log.info(
                "{finished} out of {total} workflows are finished.".format(
                    finished=finished_jobs, total=total_runs
                )
            )
        background_task_process.stop = True

        self.benchmark_results = {
            "warm": {self.destinations[0].name: {self.workflows[0].name: results}}
        }
        ...
        def run(self):
            ...
            while True:
                for task in self.bm.background_tasks:
                    ...
                    if "run_until" in task:
                        if task["run_until"] <= time.monotonic() and task[
                            "next_run"
                        ] < float("inf"):
                            task["next_run"] = float("inf")
                            log.info(
                                "Stopped background task {task}, as run_until passed".format(
                                    task=task
                                )
                            )
                if self.stop:
    ...
    class BurstThread(threading.Thread):
        ...
        def run(self):
            ...
            """
            log.info(
                "Running with thread_id {thread_id}".format(thread_id=self.thread_id)
            )
            if self.bm.destination_type is PulsarMQDestination:
                try:
                    res = BurstBenchmark.run_galaxy_benchmark(
                        self,
                        self.bm.galaxy,
                        self.bm.destinations,
                        self.bm.workflows,
                        1,
                        "warm",
                        False,
                    )
                    self.results[self.thread_id] = res[self.bm.destinations[0].name][
                        self.bm.workflows[0].name
                    ][
                        0
            ...
            if self.bm.destination_type is CondorDestination:
                for destination in self.bm.destinations:
                    for workflow in self.bm.workflows:
                        ...
                        result["history_name"] = str(time.time_ns()) + str(
...
def configure_benchmark(
    bm_config: Dict, destinations: Dict, workflows: Dict, glx, benchmarker
) -> BaseBenchmark:
    """
    ...
    if bm_config["type"] == "ColdvsWarm":
        ...
        if "cold_pre_task" in bm_config:
            if bm_config["cold_pre_task"]["type"] == "AnsiblePlaybook":
                benchmark.cold_pre_task = AnsiblePlaybookTask(
                    benchmark.destinations, bm_config["cold_pre_task"]["playbook"]
                )

        if "warm_pre_task" in bm_config:
            if bm_config["warm_pre_task"]["type"] == "AnsiblePlaybook":
                benchmark.warm_pre_task = AnsiblePlaybookTask(
                    benchmark.destinations, bm_config["warm_pre_task"]["playbook"]
                )

    if bm_config["type"] == "DestinationComparison":
        warmup = True if "warmup" not in bm_config else bm_config["warmup"]
        benchmark = DestinationComparisonBenchmark(
            bm_config["name"],
            benchmarker,
            _get_needed_destinations(
                bm_config, destinations, DestinationComparisonBenchmark
            ),
            _get_needed_workflows(bm_config, workflows, DestinationComparisonBenchmark),
            glx,
            repetitions,
            warmup,
        )
    ...
    if "pre_tasks" in bm_config:
        ...
        for task in bm_config["pre_tasks"]:
            if task["type"] == "AnsiblePlaybook":
                benchmark.pre_tasks.append(
                    AnsiblePlaybookTask(benchmark.destinations, task["playbook"])
                )
            elif task["type"] == "BenchmarkerTask":
                benchmark.pre_tasks.append(
                    BenchmarkerTask(benchmark.destinations, task["name"])
                )
            else:
                raise ValueError(
                    "Task of type '{type}' is not supported".format(type=task["type"])
                )
...
def _get_needed_destinations(bm_config: Dict, destinations: Dict, bm_type) -> List:
    ...
    """
    if (
        "destinations" not in bm_config
        or bm_config["destinations"] is None
        or len(bm_config["destinations"]) == 0
    ):
        raise ValueError(
            "No destination set in benchmark '{name}'".format(name=bm_config["name"])
        )
    ...
    for dest_name in bm_config["destinations"]:
        ...
        if dest_name not in destinations:
            raise ValueError(
                "Destination '{name}' not set in workflows-configuration.".format(
                    name=dest_name
                )
            )
        ...
        if type(destinations[dest_name]) not in bm_type.allowed_dest_types:
            raise ValueError(
                "Destination-Type {dest} is not allowed in benchmark-type {bm}. \
                             Error in benchmark name {bm_name}".format(
                    dest=type(destinations[dest_name]),
                    bm=type(bm_type),
...
def _get_needed_workflows(bm_config: Dict, workflows: Dict, bm_type) -> List:
    ...
    """
    if (
        "workflows" not in bm_config
        or bm_config["workflows"] is None
        or len(bm_config["workflows"]) == 0
    ):
        raise ValueError(
            "No workflow set in benchmark '{name}'".format(name=bm_config["name"])
        )
    ...
    for wf_name in bm_config["workflows"]:
        ...
        if wf_name not in workflows:
            raise ValueError(
                "Workflow '{name}' not set in workflows-configuration.".format(
                    name=wf_name
                )
            )
        ...
        if type(workflows[wf_name]) not in bm_type.allowed_workflow_types:
            raise ValueError(
                "Workflow-Type {wf} is not allowed in benchmark-type {bm}. \
                                         Error in benchmark name {bm_name}".format(
                    wf=type(workflows[wf_name]),
                    bm=type(bm_type),
                    bm_name=bm_config["name"],
                )
            )
